...
def about():
    ...
    #pass this as parameter to render html, which accesses param as 'var'
    # return render_template("index.html", var=form)
    return render_template("about.html", var=form, path=url_for('index'))
...
def CPTresults():
    ...
    if request.method=="POST":
        ...
        return render_template("table.html", res=res, path=url_for('index'));
...
def user_account():
    ...
    if request.method == 'GET':
        form = MyForm()
        form2 = MyFormUserAccountUpdateDoctor()
        ...
        #pass this as parameter to render html, which accesses param as 'var'
        return render_template("user_account.html", usr=u,ins=cRes[0],ph=cRes[1],dn=cRes[2],var=form, var2=form2, path=url_for('index'))
        ...
        if t[0] == 'insInput':
            ...
            # form = MyForm()
            # form2 = MyFormUserAccountUpdateDoctor()
            cur.execute("UPDATE users SET insProvider=? WHERE email=?", (insName, u))
            con.commit()
            con.close()
        elif t[0] == 'hosInput':
            u=flask_login.current_user.id
            hosName = request.form['hosInput']
            #return str(insName)
            ...
            # form = MyForm()
            # form2 = MyFormUserAccountUpdateDoctor()
            cur.execute("UPDATE users SET prefHospital=? WHERE email=?", (hosName, u))
            con.commit()
            con.close()
            
        elif t[0] == 'docInput':
            u=flask_login.current_user.id
            docName = request.form['docInput']
            #return str(insName)
            ...
            # form = MyForm()
            # form2 = MyFormUserAccountUpdateDoctor()
            cur.execute("UPDATE users SET doctorName=? WHERE email=?", (docName, u))
            con.commit()
            con.close()
        return redirect(url_for('user_account')) #render_template("user_account.html", usr=u,ins=cRes[0],ph=cRes[1],dn=cRes[2],var=form, var2=form2, path=url_for('index'))
...
class MyFormUserAccountUpdateDoctor(FlaskForm):
    name = StringField('Enter Insurer', validators=[DataRequired()])
    submit = SubmitField()
...
def addUser(name, passw):
    ...
    if usr == None:
        ...
        con = sqlite3.connect("DB_Setup/login.db")
        cur = con.cursor()
        cur.execute("INSERT INTO users VALUES (?, ?, null, null, null);",(name, passw))
        con.commit()
        con.close()
