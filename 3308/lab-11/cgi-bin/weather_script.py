...
#Sources: https://www.geeksforgeeks.org/how-to-read-a-json-response-from-a-link-in-python/
...
#This py script colorizes states by current temperature (today) by getting temp from the NWS
#CHANGE THIS VARIABLE TO RUN FOR MORE STATES:************************************************************************
stateRange=10

#Script works by creating a dict of state abbreviations: latitude,longitude,
#Then accessing the NWS API to generate a dict of abbr.:temperature
#Then uses a loop to color each state by temp.
#This had to be done in the .css section because the states are classes.
...
#CO test code
'''
#Test getting Boulder, CO daily forecast
#First, get a response from the latitude longitude coordinates.
response = urlopen('https://api.weather.gov/points/39.0646,-105.3272')
#Store that as json data
data = json.load(response)   
#Get the state
state=data["properties"]["relativeLocation"]["properties"]["state"]

#From this get the correct forecast html
forecastHTML=data["properties"]["forecast"]
#get a response from this html
response2=urlopen(forecastHTML)
#store as json data
forecastData=json.load(response2)
#print(forecastData)
#Now extract the temperature for today:
...
'''
##############################################################
#Code to extract latitude and longitude for each state,
#and create a dictionary of abbreviation:latitude,longitude

#Now get Json data of states with lat/long
latResponse=urlopen("https://gist.githubusercontent.com/meiqimichelle/7727723/raw/0109432d22f28fd1a669a3fd113e41c4193dbb5d/USstates_avg_latLong")
latData=json.load(latResponse)
#print(latData)
#Get state abbreviation data
abrResponse=urlopen("https://gist.githubusercontent.com/mshafrir/2646763/raw/8b0dbb93521f5d6889502305335104218454c2bf/states_titlecase.json")
abrData=json.load(abrResponse)
#print(abrData)
#This will store (state:lat,long.)
stateDict={}

for i in latData:
    stateDict[i['state']]= str(i['latitude'])+","+str(i['longitude'])
    
#store dict of (abr:lat,long)
finDict={}
#loop through each line of abbr
for i in abrData:
    #loop through each line of states:lat,lon
    for j in stateDict:
        #check that full state names equal bewteen the two
        if(i['name']==j):
            #create new dict entry 
            finDict[i['abbreviation']]=stateDict[j]
##############################################################################################
#Code to only store abr:temperature for <stateRange> number of states
tempD={}

for i in islice(finDict, stateRange):
    try:
        latLon=finDict[i]
        #print(latLon)
        r='https://api.weather.gov/points/'+latLon
        response=urlopen(r)
        data = json.load(response) 
        forecastHTML=data["properties"]["forecast"]
        #get a response from this html
        ...
        forecastData=json.load(response2)
        #Now extract the temperature for today:
        temp=forecastData["properties"]["periods"][0]["temperature"]
        tempD[i]=temp  
    #loop to add color for missing states (500 server error, problem on side of server)
    except:
        #print("Error in state :"+str(i))
        #print(r)
        #print(forecastHTML)
        #print()
        #make a temp for error states
        tempD[i]=1000
...
#########################################################################
#Code to get temperature for all 50 states
#ALternatively, change range of function above (variable at start of code)
#Uncomment to run for all 50 states (takes about 10-20 seconds)
...
for i in finDict:
    try:
        ...
        temp=forecastData["properties"]["periods"][0]["temperature"]
        tempD[i]=temp  
    #loop to add missing states (500 server error, problem on side of server)
    except:
        #print("Error in state :"+str(i))
        #print(r)
        #print(forecastHTML)
...
########################################################
#Code to save dict as json file
#write to json file
#with open('data3.json', 'w') as f:
    #json.dump(tempD, f)

'''
#open stored json file into dict
f=open('cgi-bin/data3.json', "r")
...
#Assign color based on temp to dict
#Then create string to later color by class
colors={}
#for i in temps:
for i in tempD:
    #t=int(temps[i])
    ...
    if(t<10):
        ...
        x="Red"
    #Make unknown temp color grey (1000 here)
    else:
        x="Gray"
    colors[i]=x
#make an array for state color filled
strs=[]
for i in colors:
    x="."+i+"{fill:"+colors[i]+"}"
    strs.append(x)
...
print('''<html>
    <head>
<title>Weather Map Lab by Mavromatis</title>
</head>''')
print("<body>")
print('''<header>
<script>
...
print('''<svg xmlns="http://www.w3.org/2000/svg" width="959" height="593">
...
''')
###########################################################################################################################
#loop through state:color string, setting each class
for s in strs:
    print(s)
print('''</style>
##############################################################################################################################
