...
# FIXED Character Counting Module
...
class CharacterCounter:
    ...
    def count_alpha(self):
        ...
        #added capital letter detection
    ...
    def is_phonenumber(self):
        ...
        """
        #fixed 9 digit numbers ret True, and multiple delimiters ret True
        #first digit can't be zero.
        #NOTE: \1 matches the exact term matched by the first () capturing group
        #So -,. can't be mixed and matched.
        #phonenum = re.compile(r'^[1-9][0-9]{2}([\-.])?[0-9]{3}\1?[0-9]{4}|\([1-9][0-9]{2}\)[0-9]{3}([\-.])?[0-9]{4}$')
        phonenum = re.compile(r'^([1-9][0-9]{2}([\-.])?[0-9]{3}\2?[0-9]{4})$|^(\([1-9][0-9]{2}\)[0-9]{3}([\-.])?[0-9]{4})$')
